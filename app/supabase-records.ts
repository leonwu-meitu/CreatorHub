...
const isManualAnalyticsStatus = (value?:string) => String(value || "").toLowerCase().replaceAll(" ", "_") === "manual_override";
...
export async function analyzeSubmissionRecord(client:SupabaseClient, submissionId:string, force=false) {
  ...
  return {
    ...
    analyticsLikes:Number(row.analytics_likes||0),
    analyticsComments:Number(row.analytics_comments||0),
    analyticsReposts:Number(row.analytics_reposts||0),
    analyticsShares:Number(row.analytics_shares||0),
    analyticsFavorites:Number(row.analytics_favorites||0),
...
export async function updateSubmissionRecord(client:SupabaseClient, submission:Submission) {
  ...
  if(error)throw error;
  const savedRate=Number(submission.engagementRate);
  if(!isManualAnalyticsStatus(submission.analyticsStatus)||submission.engagementRate==null||!Number.isFinite(savedRate)||savedRate<0||savedRate>100){
    const {error:cleanupError}=await client.from("campaign_submissions").update({engagement_rate:null}).eq("id",submission.id);
