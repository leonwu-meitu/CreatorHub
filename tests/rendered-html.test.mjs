...
test("keeps AI analytics extraction server-side and Team-reviewed", async () => {
  ...
  assert.match(edgeFunction, /recognizedCrossPlatform/);
  assert.match(edgeFunction, /booleanValue\(extracted\.valid_analytics_screenshot\)/);
  assert.match(edgeFunction, /const analyticsStatus = "ai_needs_review"/);
  ...
  assert.match(edgeFunction, /engagementExtractionPrompt\(normalizedDetectedPlatform === "unknown" \? submission\.platform : detectedPlatform\)/);
