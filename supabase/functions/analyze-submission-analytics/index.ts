...
- Instagram titles/icons: Likes = heart icon, Comments = speech-bubble icon, Shares = paper-plane icon, and Saves/Favorites = bookmark icon. On Instagram icon-only post insights, the top interaction row is ordered heart → comment bubble → paper plane → circular repost arrows → bookmark; for example, the number immediately under the heart is Likes, never Viewers. If labels are absent, read the number directly paired with each icon and never copy a number from the Summary card labeled Views or Viewers. The circular two-arrow Repost icon is excluded. For Instagram, the required formula is exactly (Likes + Comments + Shares + Saves/Favorites) / Views * 100. Ignore Profile activity, watch time, and any displayed interaction total.
