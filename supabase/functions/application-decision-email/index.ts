...
const emailHtml = ({
  ...
  return `<!doctype html><html><body style="margin:0;background:#f7f4f1;font-family:Arial,sans-serif;color:#1d1720"><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:40px 16px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fff;border:1px solid #eadfe4;border-radius:20px;overflow:hidden"><tr><td style="height:8px;background:#f11d48"></td></tr><tr><td style="padding:38px"><p style="margin:0 0 12px;color:#f11d48;font-size:12px;font-weight:700;letter-spacing:2px">CREATORHUB</p><h1 style="margin:0 0 20px;font-size:30px">Update pendaftaran Creator Pool</h1><p style="font-size:16px;line-height:1.7">Hai ${safeName}, setelah ditinjau oleh tim, pendaftaranmu belum dapat kami terima saat ini.</p><div style="margin:24px 0;padding:18px;border-radius:14px;background:#fff2f4"><strong>Alasan dari tim:</strong><p style="margin:8px 0 0;line-height:1.6">${safeReason || "Pendaftaran belum memenuhi kriteria program saat ini."}</p></div><p style="font-size:16px;line-height:1.7">Silakan periksa CreatorHub untuk melihat status terbaru.</p><p style="margin:30px 0"><a href="${safePortalUrl}" style="display:inline-block;padding:14px 24px;border-radius:999px;background:#1d1720;color:#fff;text-decoration:none;font-weight:700">Open CreatorHub</a></p></td></tr></table></td></tr></table></body></html>`;
};
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const subject = application.status === "accepted"
      ...
      : "Update pendaftaran Creator Pool kamu";
